...
///////////////////////////////////////////////////////////////////////////////
// Демонстрационный пример для графического индикатора МЭЛТ MT-12232D с
// разрешением 122x32 на базе контроллера SED1520DOA (КБ145ВГ4).
// Управление индикатором осуществляется по параллельному интерфейсу с помощью
// Arduino NANO. Прошивка производится с помощью программы Avrdude либо
// XLoader. Можно также использовать программатор AVRISP mkII.
// Подключение индикатора к Arduino NANO представлено в файле
// MT12232D-ARDUINO_NANO.pdf
///////////////////////////////////////////////////////////////////////////////
...
PT_THREAD(Task_Fonts(struct pt *Context))
{
	...
	LCD_Clear(0, 8, LCD_X_RES - 1, LCD_Y_RES - 1);

	size = LCD_X_RES / FONT_Width(FONT_8x8) - 5 + 1;
	Buf[size] = '\0';

	// Вывод названий демонстрируемых шрифтов
	LCD_Str_P(0, 12, FONT_6x8, PSTR("8x8: "), 0);
	LCD_Str_P(0, 22, FONT_6x8, PSTR("8x8n:"), 0);
	...
	while (TRUE)
	{
		...
		LCD_Str(30, 12, FONT_8x8, Buf, 0);

		PT_YIELD(Context); // Отдать управление в планировщик

		// Выводим символы шрифта 8x8n
		LCD_Str(30, 22, FONT_8x8n, Buf, 0);
	...
	// Демонстрация шрифтов 8x16, 10x14, 12x16 и шрифта 6x8 с удвоенной
	// шириной и высотой.
	///////////////////////////////////////////////////////////////////////////
	...
	while (TRUE)
	{
		...
		{
			// Установка таймаута 5000мс для следующей смены шрифта
			...
			{
				case 0:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("8x16:  "), 0);
					break;
				case 1:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("8x16n: "), 0);
					break;
				case 2:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("8x16t: "), 0);
					break;
				case 3:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("8x16g: "), 0);
					break;
				case 4:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("10x14: "), 0);
					break;
				case 5:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("12x16: "), 0);
					break;
					// Шрифт 6x8 с удвоением по ширине и высоте
				default:
					LCD_Str_P(0, 12 + 4, FONT_6x8, PSTR("6x8WH: "), 0);
					break;
			}
		}
		...
		if (curFont >= 6)
			// Шрифт 6x8 с удвоением по ширине и высоте
			LCD_Str(42, 12, idFont, Buf, LCD_TWICE_WIDTH | LCD_TWICE_HEIGHT);
		else	
			LCD_Str(42, 12, idFont, Buf, 0);
	...
	for (cnt = 0; cnt < 3; cnt++)
	{
		...
		{
			// Вычисляем случайную координату x1 в диапазоне 0...121
			x1 = random() % LCD_X_RES;
			...
			// После каждых 8-ми проходов цикла отдаем управление в
			// планировщик
	...
	for (cnt = 0; cnt < 4; cnt++)
	{
		...
		case 1:
			...
			LCD_Rect(0, 9, LCD_X_RES - 1, LCD_Y_RES - 1, TRUE);
		...
		if (cnt < 3)
		{
			...
			// Вычисляем случайную координату x2 в диапазоне 0...121
			x2 = random() % LCD_X_RES;
	...
	{
		// Рисуем прямоугольник
		LCD_Rect(61 - i * 5, 20 - i, 61 + i * 5, 20 + i, TRUE);
...
PT_THREAD(Task_Blink(struct pt *Context))
{
	...
	// Настройка вывода, к которому подключен светодиод для работы в
	// качестве выхода
	DRIVER(HL, OUT);
	...
	while (TRUE)
	{
		for (i = 0; i < 2; i++)
		{
			ON(HL); // Включаем светодиод
			MT_SleepMs(Context, 50); // Спим 50мс
			OFF(HL); // Выключаем светодиод
			MT_SleepMs(Context, 150); // Спим 150мс
		}

		MT_SleepMs(Context, 600); // Спим 600мс
	}
